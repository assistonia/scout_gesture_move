...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            Image,
            '/oakd/rgb/preview/image_raw',  # 실제 사용 중인 ROS 토픽으로 변경해주세요
                ...
                if current_time - image_subscriber.last_action_time > image_subscriber.action_cooldown and not image_subscriber.is_moving:
                    ...
                    if this_action in ['Move Forward', 'Move Backward', 'Move Right', 'Move Left']:
                        ...
                        if pending_action == 'Move Forward':
                            ...
                            threading.Thread(target=image_subscriber.move_robot_for_duration, args=(0, 0, -1.57, 5.0)).start()
                            time.sleep(2)
                            threading.Thread(target=image_subscriber.move_robot_for_duration, args=(1.0, 0, 0, 2.0)).start()
